import { pgTable, varchar, char, integer, pgEnum, boolean } from "drizzle-orm/pg-core";
...
export const DetallesVoluntarios = pgTable("DetallesVoluntarios", {
  idVoluntario: integer("idVoluntario").references(() => Voluntarios.id, {onUpdate:'cascade', onDelete: 'cascade'}).primaryKey(),
  tipoSangre: tipoSangreEnum("tipoSangre").notNull(),
  estadoCivil: estadoCivilEnum("estadoCivil").notNull(),
  telefonos: varchar({length: 12})
  .array()
  .notNull()
  .default([]), 
  ...
  telefonoContactoEmergencia: varchar("telefonoContactoEmergencia", {length: 12}),
