...
export const RegistranObservaciones = pgTable('RegistranObservaciones', {
    idUsuario: integer('idUsuario').notNull().references(() => Usuarios.id, {
        onUpdate: 'cascade',
        onDelete: 'restrict',
    }),
    idVoluntario: integer('idVoluntario').notNull().references(() => Voluntarios.id, {
        onUpdate: 'cascade',
        onDelete: 'restrict',
    }),
    fechaHoraRegistro: timestamp('fechaHoraRegistro', { withTimezone: false }).notNull().default(sql`now()`),
    observacion: varchar('observacion', { length: 200 }).notNull(),
}, (table) => {
    return {
        pk: {
            name: 'registran_observaciones_pk',
            columns: [table.idUsuario, table.idVoluntario, table.fechaHoraRegistro],
        },
    }
})
